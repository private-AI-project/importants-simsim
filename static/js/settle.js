...
(function () {
  ...
  function names(nodes) {
    return [].map.call(nodes, function (el) { return el.value; });
  }
  function sameList(a, b) {
    return a.length === b.length && a.every(function (v, i) { return v === b[i]; });
  }

  // 2단계 폼은 사용자가 손으로 만든 상태를 담고 있다. 명단이 그대로면 다시
  // 만들지 않는다. 매번 새로 만들면 금액 단위를 바꾸는 것처럼 관계없는 동작에도
  // 체크와 결제자가 초기화돼, 2차에서 뺀 사람이 슬그머니 되살아난다.
  function syncPayer() {
    if (sameList(names(elPayer.options), people)) return;
    var keep = elPayer.value;
    elPayer.innerHTML = people.map(function (n) {
      return '<option value="' + esc(n) + '">' + esc(n) + "</option>";
    }).join("");
    if (people.indexOf(keep) >= 0) elPayer.value = keep;
  }

  function syncMembers() {
    var inputs = elMembers.querySelectorAll("input");
    if (sameList(names(inputs), people)) return;

    // 껐던 사람은 껐던 대로 둔다. 새로 들어온 사람만 켠다.
    // 대개 다 참여하고 빠지는 사람만 끄기 때문이다.
    var was = {};
    [].forEach.call(inputs, function (c) { was[c.value] = c.checked; });

    elMembers.innerHTML = people.map(function (n) {
      var on = was[n] === false ? "" : " checked";
      return '<label class="st-chk"><input type="checkbox" value="' + esc(n) +
        '"' + on + '><span>' + esc(n) + "</span></label>";
    }).join("");
  }
  ...
  function addItem() {
    ...
    // 0원은 넣어도 정산에 아무 영향이 없고 목록에 빈 줄만 남는다.
    if (amount === null || amount <= 0) { elAmount.select(); return; }
